...
def process(gt_name: str, image_name: str, mode: str):
    ...
    if np.sum(gt) > 0: # has at least 1 object
        ...
        if mode == "train":
            image_data_m = io.imread(join(args.img_path+'m', image_name))
        elif mode == "valid":
            image_data_m = io.imread(join(args.img_path.replace("train", "valid")+'m', image_name))
        else:
            img_path = f"data/test_data/{args.task_name}/images"
            image_data_m = io.imread(join(img_path, image_name))
        image_ori_size = image_data_m.shape[:2]
        # Remove any alpha channel if present.
        if image_data_m.shape[-1] > 3 and len(image_data_m.shape) == 3:
            image_data_m = image_data_m[:, :, :3]
        # If image is grayscale, then repeat the last channel to convert to rgb
        if len(image_data_m.shape) == 2:
            image_data_m = np.repeat(image_data_m[:, :, None], 3, axis=-1)
        # nii preprocess start (clip the intensity)
        lower_bound, upper_bound = np.percentile(image_data_m, 0.95), np.percentile(
            image_data_m, 99.5 # Intensity of 0.95% pixels in image_data lower than lower_bound
                             # Intensity of 99.5% pixels in image_data lower than upper_bound
        )
        image_data_pre_m = np.clip(image_data_m, lower_bound, upper_bound)
        # min-max normalize and scale
        image_data_pre_m = (
            (image_data_pre_m - np.min(image_data_pre_m))
            / (np.max(image_data_pre_m) - np.min(image_data_pre_m))
            * 255.0
        )
        image_data_pre_m[image_data_m == 0] = 0 # ensure 0-255
        image_data_pre_m = np.uint8(image_data_pre_m)
        dsms.append(image_data_pre_m)

        # resize image to 3*1024*1024
        resize_img_m = sam_transform.apply_image(image_data_pre_m, interpolation=InterpolationMode.BILINEAR) # ResizeLong
        resize_img_tensor_m = torch.as_tensor(resize_img_m.transpose(2, 0, 1))[None, :, :, :].to(
            args.device
        ) # (1, 3, resized_h, 1024)
        resized_size_be_padding_m = tuple(resize_img_tensor_m.shape[-2:])
        input_image_m = sam_model.preprocess(resize_img_tensor_m) # padding to (1, 3, 1024, 1024)
        assert input_image_m.shape == (
            1,
            3,
            sam_model.image_encoder.img_size,
            sam_model.image_encoder.img_size,
        ), "input image should be resized to 1024*1024"
        assert input_image_m.shape[-2:] == (1024, 1024)
        # pre-compute the image embedding
        if mode != "train":
            sam_model.eval()
        with torch.no_grad():
            embedding_dsm = sam_model.image_encoder(input_image_m)
            embedding_dsm = embedding_dsm.cpu().numpy()[0]
            # print(embedding_dsm.shape)
            
        return gt_, new_lab_list, img_embedding, embedding_dsm, resized_size_be_padding, image_ori_size
    else:
        print(mode, gt_name)
    ...
    if args.csv != None:
        ...
        for gt_name in tqdm(names):
            ...
            if not os.path.exists(image_path):
                continue
            gt_, new_lab_list, img_embedding, embedding_dsm, resized_size_be_padding, image_ori_size = process(gt_name, img_name, mode=mode)
            if gt_ is not None:
                np.savez_compressed(
                    ...
                    img_embeddings_dsm=embedding_dsm,
