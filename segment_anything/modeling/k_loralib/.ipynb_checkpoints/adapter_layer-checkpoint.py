    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.lora_moe_experts1 = nn.ModuleList([])
            self.lora_moe_experts2 = nn.ModuleList([])
            self.lora_moe_experts3 = nn.ModuleList([])
            ...
            for upsample_ratio in self.upsample_ratios:
                ...
                expert1 = nn.Conv2d(in_channels=r, out_channels=r, kernel_size=5, stride=1, padding=2, bias=True)
                expert1.bias.data.zero_()
                self.lora_moe_experts1.append(nn.Sequential(expert1, nn.GELU()))
                
                expert2 = nn.Conv2d(in_channels=r, out_channels=r, kernel_size=7, stride=1, padding=3, bias=True)
                expert2.bias.data.zero_()
                self.lora_moe_experts2.append(nn.Sequential(expert2, nn.GELU()))
                
                expert3 = nn.Conv2d(in_channels=3*r, out_channels=r, kernel_size=1, stride=1, padding=0, bias=True)
                expert3.bias.data.zero_()
                self.lora_moe_experts3.append(nn.Sequential(expert3, nn.GELU()))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0:
            ...
            for i in range(self.num_experts):
                ...
                cur_res0 = self.lora_moe_experts[i](cur_res)
                cur_res1 = self.lora_moe_experts1[i](cur_res)
                cur_res2 = self.lora_moe_experts2[i](cur_res)
                cur_res = self.lora_moe_experts3[i](torch.cat([cur_res0, cur_res1, cur_res2], dim=1))
                if upsample_ratio != 1:
...
class ConvLoRALinear1(nn.Linear, LoRALayer):
    """
    Conv-LoRA incorporated in Linear Layer. Weights of linear layer are set to be frozen per default.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            self.lora_A_y = nn.Parameter(self.weight.new_zeros((r, in_features)))
            self.lora_B_y = nn.Parameter(self.weight.new_zeros((out_features, r)))
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
            ...
            # MoE-Conv
            topk = 1
            self.lora_moe_gating_y = MoEGate(M=conv_lora_expert_num, d=self.r, K=topk)
            self.lora_moe_experts_y = nn.ModuleList([])
            self.lora_moe_experts1_y = nn.ModuleList([])
            self.lora_moe_experts2_y = nn.ModuleList([])
            self.lora_moe_experts3_y = nn.ModuleList([])
            self.upsample_ratios_y = list(range(1, conv_lora_expert_num + 1))
            for upsample_ratio in self.upsample_ratios:
                expert_y = nn.Conv2d(in_channels=r, out_channels=r, kernel_size=3, stride=1, padding=1, bias=True)
                expert_y.bias.data.zero_()
                self.lora_moe_experts_y.append(nn.Sequential(expert_y, nn.GELU()))
                
                expert1 = nn.Conv2d(in_channels=r, out_channels=r, kernel_size=5, stride=1, padding=2, bias=True)
                expert1.bias.data.zero_()
                self.lora_moe_experts1_y.append(nn.Sequential(expert1, nn.GELU()))
                
                expert2 = nn.Conv2d(in_channels=r, out_channels=r, kernel_size=7, stride=1, padding=3, bias=True)
                expert2.bias.data.zero_()
                self.lora_moe_experts2_y.append(nn.Sequential(expert2, nn.GELU()))
                
                expert3 = nn.Conv2d(in_channels=3*r, out_channels=r, kernel_size=1, stride=1, padding=0, bias=True)
                expert3.bias.data.zero_()
                self.lora_moe_experts3_y.append(nn.Sequential(expert3, nn.GELU()))
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        result = F.linear(x, self.T(self.weight), bias=self.bias)
        result_y = F.linear(y, self.T(self.weight), bias=self.bias)
        if self.r > 0:
            lora_res = self.lora_dropout(x) @ self.lora_A.T
            dim = lora_res.dim()
            if dim == 3:
        ...
        if self.r > 0:
            lora_res_y = self.lora_dropout(y) @ self.lora_A_y.T
            dim = lora_res_y.dim()
            if dim == 3:
                B, L, C = lora_res_y.size()
                H = W = int(math.sqrt(L))
                lora_res_y = lora_res_y.reshape(B, H, W, C)
            else:
                H, W = lora_res_y.size()[1:3]

            # Calculate the gating values.
            lora_res_y = lora_res_y.permute(0, 3, 1, 2).contiguous()
            gates_y, moe_loss = self.lora_moe_gating_y(lora_res_y)

            # Distribute data samples to experts.
            dispatcher_y = SparseDispatcher(self.num_experts, gates_y)
            expert_inputs_y = dispatcher_y.dispatch(lora_res_y)
            expert_outputs_y = []
            for i in range(self.num_experts):
                if len(expert_inputs_y[i]) == 0:
                    continue
                upsample_ratio_y = self.upsample_ratios_y[i]
                cur_res_y = expert_inputs_y[i]
                if upsample_ratio_y != 1:
                    cur_res_y = F.interpolate(cur_res_y, scale_factor=upsample_ratio_y, mode="bicubic")
                # cur_res_y = self.lora_moe_experts_y[i](cur_res_y)
                cur_res0_y = self.lora_moe_experts_y[i](cur_res_y)
                cur_res1_y = self.lora_moe_experts1_y[i](cur_res_y)
                cur_res2_y = self.lora_moe_experts2_y[i](cur_res_y)
                cur_res_y = self.lora_moe_experts3_y[i](torch.cat([cur_res0_y, cur_res1_y, cur_res2_y], dim=1))
                
                if upsample_ratio_y != 1:
                    cur_res_y = F.interpolate(cur_res_y, size=(int(H), int(W)), mode="bicubic")
                expert_outputs_y.append(cur_res_y)

            # Combine data samples after processing by each expert.
            temp_lora_res_y = dispatcher_y.combine(expert_outputs_y, multiply_by_gates=self.multiply_by_gates)
            lora_res_y = lora_res_y + temp_lora_res_y

            lora_res_y = lora_res_y.permute(0, 2, 3, 1).contiguous()
            if dim == 3:
                lora_res_y = lora_res_y.reshape(B, L, C)
        lora_x =  (lora_res @ self.lora_B.T) * self.scaling
        lora_y = (lora_res_y @ self.lora_B_y.T) * self.scaling
        # print(self.lora_wx.shape)
        # print(lora_res_y.shape)
        result += self.lora_wx * lora_x + (1 - self.lora_wx) * lora_y
        result_y += self.lora_wy * lora_y + (1 - self.lora_wy) * lora_x
